#' ---
#' title: "DEG late vs early beta in before and early trans"
#' output:
#'  html_document:
#'    code_folding: hide
#' ---
...
DEG.results <- list()
fgsea.results <- list()
if (file.exists(paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.pt.results.rds"))) {
  DEG.results <- readRDS(paste0("tmp_data/",TD,"/trans.before.DEG.cluster.results.rds"))
  pt.results <- readRDS(paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.pt.results.rds"))
  gsea.results <- readRDS(paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.gsea.results.rds"))
  fgsea.results <- readRDS(paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.fgsea.results.rds"))
  
}else{
  load("tmp_data/gene.meta.Rdata",verbose=T)
  meta.filter <- readRDS(paste0("tmp_data/",TD,"/meta.filter.rds")) %>% filter(devTime=="HS980_notrans_CM310")
  ...
  saveRDS(DEG.results,paste0("tmp_data/",TD,"/trans.before.DEG.cluster.results.rds"))
  saveRDS(pt.results,paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.pt.results.rds"))
  saveRDS(gsea.results,paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.gsea.results.rds"))
  saveRDS(fgsea.results,paste0("tmp_data/",TD,"/trans.before.DEG.cluster.MSigR.fgsea.results.rds"))
